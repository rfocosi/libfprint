...
/* Approx. sensor size: 15mm x 9mm => ~6.22 px/mm average. */
...
#define CB2000_LOW_VAR_RATIO_MAX       0.15
/* Area thresholds per action (can be overridden by env). */
#define CB2000_AREA_MIN_CAPTURE_PCT    85
#define CB2000_AREA_MIN_ENROLL_PCT     70
#define CB2000_AREA_MIN_VERIFY_PCT     70
...
/* NCC correlation matching */
#define CB2000_NR_ENROLL_STAGES             5
...
/*
 * Default NCC thresholds tuned for tiny 80x64 captures.
 * Keep multi-sample support enabled to avoid easy false accepts.
 */
#define CB2000_NCC_MATCH_TOP1_DEFAULT       0.20
#define CB2000_NCC_MATCH_TOP2_DEFAULT       0.15
#define CB2000_NCC_MATCH_MEAN3_DEFAULT      0.15
#define CB2000_NCC_MATCH_GRAD1_DEFAULT      0.15
#define CB2000_NCC_SUPPORT_MIN_DEFAULT      1
#define CB2000_NCC_SUPPORT_INT_DEFAULT      0.20
#define CB2000_NCC_SUPPORT_GRAD_DEFAULT     0.15
/* Medium band for borderline same-finger attempts on tiny captures. */
#define CB2000_NCC_MEDIUM_TOP1_DEFAULT      0.18
#define CB2000_NCC_MEDIUM_TOP2_DEFAULT      0.14
#define CB2000_NCC_MEDIUM_MEAN3_DEFAULT     0.14
#define CB2000_NCC_MEDIUM_GRAD1_DEFAULT     0.14
#define CB2000_NCC_MEDIUM_SUPPORT_MIN_DEFAULT 1
/* Dominant-match band: high top1 with coherent mean/gradient and minimum support. */
#define CB2000_NCC_DOMINANT_TOP1_DEFAULT    0.25
#define CB2000_NCC_DOMINANT_TOP2_DEFAULT    0.15
#define CB2000_NCC_DOMINANT_MEAN3_DEFAULT   0.15
#define CB2000_NCC_DOMINANT_GRAD1_DEFAULT   0.15
#define CB2000_NCC_DOMINANT_SUPPORT_MIN_DEFAULT 0
/*
 * Cross-sample consistency band:
 * If at least 1 enrolled sample supports the captured frame,
 * accept with lower per-score thresholds.
 */
#define CB2000_NCC_CONSISTENT_TOP1_DEFAULT   0.16
#define CB2000_NCC_CONSISTENT_MEAN3_DEFAULT  0.14
#define CB2000_NCC_CONSISTENT_GRAD1_DEFAULT  0.13
#define CB2000_NCC_CONSISTENT_SUPPORT_MIN_DEFAULT 1
#define CB2000_NCC_NOMATCH_TOP1_DEFAULT      0.10
/*
 * Single-strong band:
 * allow MATCH when one enrolled sample is clearly strong and stable.
 */
#define CB2000_NCC_SINGLE_TOP1_DEFAULT       0.28
#define CB2000_NCC_SINGLE_TOP2_DEFAULT       0.15
#define CB2000_NCC_SINGLE_MEAN3_DEFAULT      0.15
#define CB2000_NCC_SINGLE_GRAD1_DEFAULT      0.15
#define CB2000_NCC_SINGLE_SUPPORT_MIN_DEFAULT 0
/*
 * Clear no-match band:
 * avoid endless RETRY for clearly different fingers.
 */
#define CB2000_NCC_CLEAR_NOMATCH_TOP1_DEFAULT 0.12
#define CB2000_NCC_CLEAR_NOMATCH_MEAN3_DEFAULT 0.10
/* Adaptive thresholding based on enroll-template cohesion. */
#define CB2000_NCC_ADAPTIVE_ENABLED_DEFAULT 1
#define CB2000_NCC_ADAPTIVE_MAX_RELAX_DEFAULT 0.08
#define CB2000_NCC_ADAPTIVE_TARGET_MEAN_DEFAULT 0.36
#define CB2000_NCC_ADAPTIVE_TARGET_MIN_DEFAULT 0.28
#define CB2000_NCC_ADAPTIVE_SUPPORT_RELAX_AT 0.045
#define CB2000_NCC_ADAPTIVE_RELAX_SUPPORT_DEFAULT 0
...
typedef enum {
    ...
    CB2000_VERIFY_ROUTE_HOST_NBIS,
    CB2000_VERIFY_ROUTE_DEVICE_STATUS,
...
struct _FpiDeviceCanvasbio {
    ...
    /* NCC enrollment state */
    guint8       *enroll_images[5];
    gint          enroll_stage;
    ...
    /* Verify template telemetry (host-side NBIS path). */
    ...
    /* Trilha B skeleton: runtime flag for device-assisted verify experiments. */
    gboolean      experimental_device_assisted_verify;
    ...
    gboolean      verify_device_status_short_circuit;
    gboolean      verify_ready_query_enabled;
    gboolean      verify_ready_query_b_enabled;
...
G_DECLARE_FINAL_TYPE(FpiDeviceCanvasbio, fpi_device_canvasbio, FPI, DEVICE_CANVASBIO, FpDevice)
G_DEFINE_TYPE(FpiDeviceCanvasbio, fpi_device_canvasbio, FP_TYPE_DEVICE)
 ...
 * Phase 2: first capture trigger (a8:08) + quality check (a8:3e).
 * Phase 3: post-quality config (a8:20 status reads).
 * Phase 4: calibration + second capture trigger (idx=5123). */
static const Cb2000Command capture_start_cmds[] = {
    ...
    {CMD_CTRL_OUT, REQ_CONFIG, 0x0002, 4, NULL, 0},
    /* Switch to CAPTURE mode values (post-detection) */
...
/*
 * Verify-specific start sequence.
 * Kept as a dedicated table (even if currently equal to capture_start_cmds)
 * so we can evolve verify 1:1 without impacting enroll/capture flow.
 */
static const Cb2000Command verify_start_cmds[] = {
    /* Phase 2: First capture trigger + quality response */
    {CMD_CTRL_OUT, REQ_CONFIG, 0x0003, 3, NULL, 0},
    {CMD_BULK_OUT, 0, 0, 0, data_cap_read_08, 3},
    {CMD_BULK_IN,  0, 0, 0, NULL, 3},       /* First capture quality: ff:00:XX */
...
cb2000_classify_finalize_ack(FpiDeviceCanvasbio *self, guint8 *status_hint, gboolean *mismatch_out);
...
{
    const gchar *v = g_getenv("CB2000_DUMP_BIN");
    if (!v)
        return FALSE;
...
/*
 * Parse a boolean env var with explicit defaults.
 * Accepted true values: 1,true,yes,on
 * Accepted false values: 0,false,no,off
 */
static gboolean
cb2000_get_env_bool(const gchar *name, gboolean default_value)
{
    const gchar *v = g_getenv(name);
    if (!v || !*v)
        return default_value;

    if (g_ascii_strcasecmp(v, "1") == 0 ||
        g_ascii_strcasecmp(v, "true") == 0 ||
        g_ascii_strcasecmp(v, "yes") == 0 ||
...
static int
cb2000_parse_upscale_value(const gchar *v, int default_value)
{
    if (!v || !*v)
        return default_value;
...
static int
cb2000_get_env_int(const gchar *name,
                   int          default_value,
                   int          min_value,
                   int          max_value)
{
...
{
    const gchar *v_phase = NULL;
    const gchar *v_global = g_getenv("CB2000_UPSCALE");
    int action_default = 1;

    /*
     * Keep preprocess parity between template generation and matching:
     * enroll/verify/identify default to 2x, capture remains 1x.
     */
    switch (action) {
    ...
    case FPI_DEVICE_ACTION_VERIFY:
        action_default = 2;
        v_phase = g_getenv("CB2000_VERIFY_UPSCALE");
        break;
    case FPI_DEVICE_ACTION_IDENTIFY:
        action_default = 2;
        v_phase = g_getenv("CB2000_IDENTIFY_UPSCALE");
        break;
    case FPI_DEVICE_ACTION_ENROLL:
        action_default = 2;
        v_phase = g_getenv("CB2000_ENROLL_UPSCALE");
        break;
    case FPI_DEVICE_ACTION_CAPTURE:
        v_phase = g_getenv("CB2000_CAPTURE_UPSCALE");
        break;
    default:
        break;
    }
    ...
    if (v_phase && *v_phase)
        return cb2000_parse_upscale_value(v_phase, action_default);

    return cb2000_parse_upscale_value(v_global, action_default);
}

/*
 * Background subtraction helps remove fixed-pattern noise in enroll/capture.
 * Default is configured per action via env knobs, with verify/identify mapped
 * by CB2000_VERIFY_BG_SUBTRACT and others by CB2000_BG_SUBTRACT.
 */
static gboolean
cb2000_should_apply_bg_subtraction(FpiDeviceAction action)
{
    /* Keep verify/identify aligned with enroll defaults for NBIS matching. */
    if (action == FPI_DEVICE_ACTION_VERIFY)
        return cb2000_get_env_bool("CB2000_VERIFY_BG_SUBTRACT", TRUE);

    if (action == FPI_DEVICE_ACTION_IDENTIFY) {
        const gchar *v_identify = g_getenv("CB2000_IDENTIFY_BG_SUBTRACT");
        if (v_identify && *v_identify)
            return cb2000_get_env_bool("CB2000_IDENTIFY_BG_SUBTRACT", TRUE);
        return cb2000_get_env_bool("CB2000_VERIFY_BG_SUBTRACT", TRUE);
    }

    if (action == FPI_DEVICE_ACTION_ENROLL) {
        const gchar *v_enroll = g_getenv("CB2000_ENROLL_BG_SUBTRACT");
        if (v_enroll && *v_enroll)
            return cb2000_get_env_bool("CB2000_ENROLL_BG_SUBTRACT", TRUE);
        return cb2000_get_env_bool("CB2000_BG_SUBTRACT", TRUE);
    }

    if (action == FPI_DEVICE_ACTION_CAPTURE) {
        const gchar *v_capture = g_getenv("CB2000_CAPTURE_BG_SUBTRACT");
        if (v_capture && *v_capture)
            return cb2000_get_env_bool("CB2000_CAPTURE_BG_SUBTRACT", TRUE);
    }
...
{
    const char *v_ppmm = g_getenv("CB2000_PPMM");
    if (v_ppmm && *v_ppmm) {
        double ppmm = g_ascii_strtod(v_ppmm, NULL);
        if (ppmm > 0.0)
            return ppmm;
    }

    const char *v_dpi = g_getenv("CB2000_DPI");
    if (v_dpi && *v_dpi) {
        double dpi = g_ascii_strtod(v_dpi, NULL);
        if (dpi > 0.0)
            return dpi / 25.4;
    }
...
{
    const char *v = g_getenv("CB2000_PRECHECK_MINUTIAE");
    if (v && *v && strcmp(v, "0") != 0)
        return TRUE;

    if (action == FPI_DEVICE_ACTION_VERIFY)
        return cb2000_get_env_bool("CB2000_PRECHECK_VERIFY", FALSE);

    if (action == FPI_DEVICE_ACTION_IDENTIFY)
        return cb2000_get_env_bool("CB2000_PRECHECK_IDENTIFY", FALSE);

    if (action == FPI_DEVICE_ACTION_ENROLL)
        return cb2000_get_env_bool("CB2000_PRECHECK_ENROLL", FALSE);

    if (action == FPI_DEVICE_ACTION_CAPTURE)
        return cb2000_get_env_bool("CB2000_PRECHECK_CAPTURE", FALSE);
...
{
    gboolean global_default = cb2000_get_env_bool("CB2000_DEBUG_SAVE_PGM", TRUE);
    ...
    case FPI_DEVICE_ACTION_VERIFY:
        return cb2000_get_env_bool("CB2000_DEBUG_SAVE_VERIFY", global_default);
    case FPI_DEVICE_ACTION_IDENTIFY:
        return cb2000_get_env_bool("CB2000_DEBUG_SAVE_IDENTIFY", global_default);
    case FPI_DEVICE_ACTION_ENROLL:
        return cb2000_get_env_bool("CB2000_DEBUG_SAVE_ENROLL", global_default);
    case FPI_DEVICE_ACTION_CAPTURE:
        return cb2000_get_env_bool("CB2000_DEBUG_SAVE_CAPTURE", global_default);
    default:
        return global_default;
    }
}
...
{
    const gchar *out_dir = g_getenv("CB2000_OUTPUT_DIR");
    const gchar *home;
...
{
    int action_default = CB2000_AREA_MIN_CAPTURE_PCT;
    int global_min = cb2000_get_env_int("CB2000_AREA_MIN",
                                        CB2000_AREA_MIN_CAPTURE_PCT,
                                        1, 100);
    int capture_min = 0;

    /* Non-image actions still need explicit handling to keep -Wswitch-enum clean. */
    capture_min = cb2000_get_env_int("CB2000_AREA_MIN_CAPTURE", global_min, 1, 100);
    ...
    case FPI_DEVICE_ACTION_CLEAR_STORAGE:
        return capture_min;
    case FPI_DEVICE_ACTION_ENROLL:
        action_default = CB2000_AREA_MIN_ENROLL_PCT;
        global_min = cb2000_get_env_int("CB2000_AREA_MIN",
                                        action_default, 1, 100);
        return cb2000_get_env_int("CB2000_AREA_MIN_ENROLL", global_min, 1, 100);
    case FPI_DEVICE_ACTION_VERIFY:
    case FPI_DEVICE_ACTION_IDENTIFY:
        action_default = CB2000_AREA_MIN_VERIFY_PCT;
        global_min = cb2000_get_env_int("CB2000_AREA_MIN",
                                        action_default, 1, 100);
        return cb2000_get_env_int("CB2000_AREA_MIN_VERIFY", global_min, 1, 100);
    case FPI_DEVICE_ACTION_CAPTURE:
    default:
        return capture_min;
    }
...
/* Internal helper: read internal NBIS subprint cardinality for telemetry only. */
static guint
...
static void
cb2000_update_submit_telemetry(FpiDeviceCanvasbio *self,
                               FpDevice                  *dev,
...
/*
 * Trilha B skeleton:
 * runtime gate for future verify path that uses device-assisted matching.
 * Current behavior intentionally falls back to host-side NBIS matching.
 */
static gboolean G_GNUC_UNUSED
cb2000_try_device_assisted_verify(FpiDeviceCanvasbio *self,
                                  FpDevice                  *dev,
                                  FpImage                   *img)
{
    (void) img;

    if (!self->experimental_device_assisted_verify)
        return FALSE;

    switch (self->verify_ack_decision_last) {
    case CB2000_VERIFY_ACK_DEVICE_NOMATCH:
        fp_warn("[ EXP ] device-assisted verify: finalize ACK indicates no-match (status=0x%02x)",
                self->verify_ack_status_last);
        fpi_device_verify_report(dev, FPI_MATCH_FAIL, NULL, NULL);
        return TRUE;
    case CB2000_VERIFY_ACK_READY:
        fp_info("[ EXP ] device-assisted verify: ACK ready (status=0x%02x), host NBIS fallback remains active",
                self->verify_ack_status_last);
        return FALSE;
    case CB2000_VERIFY_ACK_RETRY:
        fp_info("[ EXP ] device-assisted verify: ACK retry already handled by gate");
        return FALSE;
    case CB2000_VERIFY_ACK_UNKNOWN:
    default:
        fp_info("[ EXP ] device-assisted verify: ACK unknown (status=0x%02x), using host NBIS fallback",
                self->verify_ack_status_last);
        return FALSE;
    }
...
cb2000_retry_scan_with_cause(FpDevice           *dev,
                             ...
                             const gchar        *detail)
{
    FpiDeviceCanvasbio *self = FPI_DEVICE_CANVASBIO(dev);
...
cb2000_minutiae_precheck_cb(GObject *source, GAsyncResult *res, gpointer user_data)
{
    ...
    FpiDeviceCanvasbio *self = FPI_DEVICE_CANVASBIO(ctx->dev);
    g_autoptr(GError) error = NULL;
                       ...
                       const FpiUsbTransfer *transfer)
{
    ...
    if (cmd->type != CMD_BULK_IN && cmd->type != CMD_CTRL_IN)
        return;
    if (!cb2000_get_env_bool("CB2000_DEBUG_CMD_RX", TRUE))
        return;
...
{
    switch (route) {
    case CB2000_VERIFY_ROUTE_HOST_NBIS:
        return "HOST_NCC";
    case CB2000_VERIFY_ROUTE_DEVICE_STATUS:
        return "DEVICE_STATUS";
 ...
 * handling when device status is explicit enough to skip host NBIS.
 */
static Cb2000VerifyResultClass
cb2000_classify_verify_result_codes(guint8 status_code, guint8 result_code)
{
 ...
 *   3) A=0x00 and (B missing or B=0x00) -> READY fallback to host NBIS
 *   4) otherwise -> UNKNOWN fallback to host NBIS
 */
static Cb2000VerifyReadyMatrixDecision
cb2000_decide_ready_query_matrix(guint8 status_a, gboolean has_b, guint8 status_b)
{
 ...
 */
static Cb2000VerifyAckDecision
cb2000_classify_finalize_ack(FpiDeviceCanvasbio *self,
                             guint8                   *status_hint,
 ...
 * This performs only "early exits" (retry). Match/no-match decisions for
 * experimental device-assisted flow are handled later by
 * cb2000_try_device_assisted_verify().
 */
static gboolean
                      ...
                      GError         *error)
{
    ...
    if (transfer->actual_length < 2) {
        fp_dbg("Poll: short read (%zu bytes), retrying",
               transfer->actual_length);
    ...
    fp_dbg("Poll response: len=%zu [0]=%02x [1]=%02x (hits=%d streak=%d)",
           transfer->actual_length,
           transfer->buffer[0], transfer->buffer[1],
           self->poll_stable_hits, self->no_finger_streak);
    if (self->poll_total_count == 0) {
        fp_dbg("WAIT_FINGER started: t0=%" G_GINT64_FORMAT "us",
               self->poll_start_us);
    }

    if (self->poll_total_count > 0 && (self->poll_total_count % 10) == 0) {
        fp_dbg("WAIT_FINGER timing: poll=%u t=+%" G_GINT64_FORMAT "us",
               self->poll_total_count, (now_us - self->poll_start_us));
    }
    ...
    if (finger_present) {
        ...
        if (self->poll_total_count <= CB2000_EARLY_PLACEMENT_POLLS) {
            fp_dbg("Early placement gate disabled (poll=%d)",
                   self->poll_total_count);
        }

        if (self->poll_stable_hits >= CB2000_POLL_DEBOUNCE_COUNT) {
            fp_dbg("Finger detected (stable, hits=%d)", self->poll_stable_hits);
            fpi_ssm_mark_completed(transfer->ssm);
            return;
        }

        fp_dbg("Finger detected but debouncing (%d/%d)",
               self->poll_stable_hits, CB2000_POLL_DEBOUNCE_COUNT);
    } else {
...
poll_finger_run_state(FpiSsm *ssm, FpDevice *dev)
{
    ...
    case POLL_FINGER_SEND:
        fp_dbg("Polling REQ_POLL (wValue=0x0007 wIndex=0 len=2)");
                       ...
                       GError         *error)
{
    ...
    gboolean finger_present = (transfer->buffer[1] != 0);
    fp_dbg("Removal poll: [0]=%02x [1]=%02x finger_present=%d",
           transfer->buffer[0], transfer->buffer[1], finger_present);
    ...
    if (finger_present) {
        ...
        if (self->removal_stable_off_hits < CB2000_REMOVAL_STABLE_OFF_COUNT) {
            fp_dbg("Removal debounce: off hits %d/%d",
                   self->removal_stable_off_hits,
                   CB2000_REMOVAL_STABLE_OFF_COUNT);
            fpi_ssm_jump_to_state(transfer->ssm, POLL_REMOVAL_DELAY);
            return;
        }

        fp_dbg("Finger removed, reporting status");
        if (!self->deactivating && !self->deactivation_in_progress)
            fpi_ssm_mark_completed(transfer->ssm);
    }
}
 ...
 * important preprocessing step for NBIS minutiae detection on small sensors:
    ...
    for (gint ty = 0; ty < tiles_y; ty++) {
        for (gint tx = 0; tx < tiles_x; tx++) {
            ...
            for (gint i = 0; i < 256; i++) {
                cumulative += hist[i];
                lut[tile_idx * 256 + i] =
                    (guint8)((cumulative * 255) / tile_size);
            }
        }
    }
 ...
 * more natural ridge contours for NBIS.
 */
static void
               ...
               GError         *error)
{
    ...
    payload_offset = (gsize) cb2000_get_env_int("CB2000_CHUNK_PAYLOAD_OFFSET",
...
/* ============================================================================
 * NCC CORRELATION MATCHER (V33 real, robust decision)
 * ============================================================================
 * Match strategy:
 * 1) Intensity NCC over small 2D shifts on foreground-only mask.
 * 2) Gradient-magnitude NCC over the same masked support.
 * 3) Conservative decision matrix using top1/top2/mean(top3)/support_count.
 */

typedef enum {
    CB2000_NCC_DECISION_MATCH = 0,
    CB2000_NCC_DECISION_NO_MATCH,
    CB2000_NCC_DECISION_RETRY,
    CB2000_NCC_DECISION_ERROR,
} Cb2000NccDecision;

typedef struct {
    gdouble best_intensity;
    gdouble second_intensity;
    gdouble mean_top3_intensity;
    gdouble grad_for_best_intensity;
    guint   support_count;
    guint   compared_images;
    gdouble enroll_pair_mean_intensity;
    gdouble enroll_pair_min_intensity;
    gdouble enroll_pair_mean_gradient;
    guint   enroll_pair_count;
    gdouble adaptive_relax;
} Cb2000NccTelemetry;

static const char *
cb2000_ncc_decision_label(Cb2000NccDecision decision)
{
    switch (decision) {
    case CB2000_NCC_DECISION_MATCH:
        return "MATCH";
    case CB2000_NCC_DECISION_NO_MATCH:
        return "NO_MATCH";
    case CB2000_NCC_DECISION_RETRY:
        return "RETRY";
    case CB2000_NCC_DECISION_ERROR:
        return "ERROR";
    default:
        return "UNKNOWN";
    }
...
static gdouble
cb2000_ncc_env_double(const char *name,
                      gdouble     fallback,
                      gdouble     min,
                      gdouble     max)
{
...
static guint
cb2000_ncc_env_uint(const char *name, guint fallback, guint min, guint max)
{
    const char *env = g_getenv(name);
    char *end = NULL;
    unsigned long val;
...
static guint
cb2000_build_foreground_mask(const guint8 *img,
                             guint8       *mask,
                             gint          width,
                             gint          height,
                             guint8        threshold)
{
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            gint idx = y * width + x;
            guint8 on = img[idx] >= threshold ? 1 : 0;
            mask[idx] = on;
            active += on;
        }
    }
...
static gdouble
cb2000_ncc_match_f64_masked(const gdouble *a,
                            const gdouble *b,
                            const guint8  *mask_a,
                            const guint8  *mask_b,
                            gint           width,
...
static gdouble
cb2000_ncc_compute_adaptive_relax(const Cb2000NccTelemetry *tel)
{
    guint adaptive_enabled = cb2000_ncc_env_uint("CB2000_NCC_ADAPTIVE",
                                                  CB2000_NCC_ADAPTIVE_ENABLED_DEFAULT,
                                                  0, 1);
    gdouble max_relax = cb2000_ncc_env_double("CB2000_NCC_ADAPTIVE_MAX_RELAX",
                                              CB2000_NCC_ADAPTIVE_MAX_RELAX_DEFAULT,
                                              0.0, 0.20);
    gdouble target_mean = cb2000_ncc_env_double("CB2000_NCC_ADAPTIVE_TARGET_MEAN",
                                                CB2000_NCC_ADAPTIVE_TARGET_MEAN_DEFAULT,
                                                0.0, 1.0);
    gdouble target_min = cb2000_ncc_env_double("CB2000_NCC_ADAPTIVE_TARGET_MIN",
                                               CB2000_NCC_ADAPTIVE_TARGET_MIN_DEFAULT,
                                               0.0, 1.0);
    gdouble deficit_mean;
    gdouble deficit_min;
    gdouble relax;

    if (!adaptive_enabled || tel->enroll_pair_count == 0)
        return 0.0;

    deficit_mean = MAX(0.0, target_mean - tel->enroll_pair_mean_intensity);
    deficit_min = MAX(0.0, target_min - tel->enroll_pair_min_intensity);

    /* Weighted deficits: broad cohesion (mean) + worst-case spread (min). */
    relax = deficit_mean * 0.60 + deficit_min * 0.40;
    if (relax > max_relax)
        relax = max_relax;
    if (relax < 0.0)
        relax = 0.0;
...
static void
cb2000_ncc_compute_enroll_cohesion(const guint8 *images_data,
                                   gint          n_images,
                                   guint8        mask_threshold,
                                   guint         min_samples,
                                   gint          max_shift,
                                   Cb2000NccTelemetry *tel)
{
    guint8 masks[CB2000_NR_ENROLL_STAGES][CB2000_IMG_SIZE];
    gdouble grads[CB2000_NR_ENROLL_STAGES][CB2000_IMG_SIZE];
    const guint8 *images[CB2000_NR_ENROLL_STAGES];
    gdouble sum_int = 0.0;
    gdouble sum_grad = 0.0;
    gdouble min_int = 1.0;
    guint pairs = 0;
    gint i, j;

    tel->enroll_pair_mean_intensity = 0.0;
    tel->enroll_pair_min_intensity = 0.0;
    tel->enroll_pair_mean_gradient = 0.0;
    tel->enroll_pair_count = 0;
    ...
    for (i = 0; i < n_images; i++) {
        images[i] = images_data + (i * CB2000_IMG_SIZE);
        cb2000_build_foreground_mask(images[i], masks[i],
                                     CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT,
                                     mask_threshold);
        cb2000_compute_gradient_magnitude(images[i], grads[i],
                                          CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT);
    }

    for (i = 0; i < n_images; i++) {
        for (j = i + 1; j < n_images; j++) {
            gdouble int_pair = cb2000_ncc_match_u8_masked(images[i], images[j],
                                                          masks[i], masks[j],
                                                          CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT,
                                                          max_shift, min_samples);
            gdouble grad_pair = cb2000_ncc_match_f64_masked(grads[i], grads[j],
                                                            masks[i], masks[j],
                                                            CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT,
                                                            max_shift, min_samples);
            sum_int += int_pair;
            sum_grad += grad_pair;
            min_int = MIN(min_int, int_pair);
            pairs++;
            fp_dbg("[ NCC ] cohesion pair=%d-%d int=%.4f grad=%.4f",
                   i, j, int_pair, grad_pair);
        }
    }
    ...
    tel->enroll_pair_count = pairs;
    tel->enroll_pair_mean_intensity = sum_int / pairs;
    tel->enroll_pair_min_intensity = min_int;
    tel->enroll_pair_mean_gradient = sum_grad / pairs;
}

static Cb2000NccDecision
cb2000_ncc_classify(const Cb2000NccTelemetry *tel)
{
    gdouble match_top1 = cb2000_ncc_env_double("CB2000_NCC_MATCH_TOP1",
                                               CB2000_NCC_MATCH_TOP1_DEFAULT,
                                               0.0, 1.0);
    gdouble match_top2 = cb2000_ncc_env_double("CB2000_NCC_MATCH_TOP2",
                                               CB2000_NCC_MATCH_TOP2_DEFAULT,
                                               0.0, 1.0);
    gdouble match_mean3 = cb2000_ncc_env_double("CB2000_NCC_MATCH_MEAN3",
                                                CB2000_NCC_MATCH_MEAN3_DEFAULT,
                                                0.0, 1.0);
    gdouble match_grad1 = cb2000_ncc_env_double("CB2000_NCC_MATCH_GRAD1",
                                                CB2000_NCC_MATCH_GRAD1_DEFAULT,
                                                0.0, 1.0);
    guint support_min = cb2000_ncc_env_uint("CB2000_NCC_SUPPORT_MIN",
                                            CB2000_NCC_SUPPORT_MIN_DEFAULT,
                                            1, CB2000_NR_ENROLL_STAGES);
    gdouble medium_top1 = cb2000_ncc_env_double("CB2000_NCC_MEDIUM_TOP1",
                                                CB2000_NCC_MEDIUM_TOP1_DEFAULT,
                                                0.0, 1.0);
    gdouble medium_top2 = cb2000_ncc_env_double("CB2000_NCC_MEDIUM_TOP2",
                                                CB2000_NCC_MEDIUM_TOP2_DEFAULT,
                                                0.0, 1.0);
    gdouble medium_mean3 = cb2000_ncc_env_double("CB2000_NCC_MEDIUM_MEAN3",
                                                 CB2000_NCC_MEDIUM_MEAN3_DEFAULT,
                                                 0.0, 1.0);
    gdouble medium_grad1 = cb2000_ncc_env_double("CB2000_NCC_MEDIUM_GRAD1",
                                                 CB2000_NCC_MEDIUM_GRAD1_DEFAULT,
                                                 0.0, 1.0);
    guint medium_support_min = cb2000_ncc_env_uint("CB2000_NCC_MEDIUM_SUPPORT_MIN",
                                                    CB2000_NCC_MEDIUM_SUPPORT_MIN_DEFAULT,
                                                    1, CB2000_NR_ENROLL_STAGES);
    gdouble dominant_top1 = cb2000_ncc_env_double("CB2000_NCC_DOMINANT_TOP1",
                                                  CB2000_NCC_DOMINANT_TOP1_DEFAULT,
                                                  0.0, 1.0);
    gdouble dominant_top2 = cb2000_ncc_env_double("CB2000_NCC_DOMINANT_TOP2",
                                                   CB2000_NCC_DOMINANT_TOP2_DEFAULT,
                                                   0.0, 1.0);
    gdouble dominant_mean3 = cb2000_ncc_env_double("CB2000_NCC_DOMINANT_MEAN3",
                                                   CB2000_NCC_DOMINANT_MEAN3_DEFAULT,
                                                   0.0, 1.0);
    gdouble dominant_grad1 = cb2000_ncc_env_double("CB2000_NCC_DOMINANT_GRAD1",
                                                   CB2000_NCC_DOMINANT_GRAD1_DEFAULT,
                                                   0.0, 1.0);
    guint dominant_support_min = cb2000_ncc_env_uint("CB2000_NCC_DOMINANT_SUPPORT_MIN",
                                                      CB2000_NCC_DOMINANT_SUPPORT_MIN_DEFAULT,
                                                      1, CB2000_NR_ENROLL_STAGES);
    gdouble consistent_top1 = cb2000_ncc_env_double("CB2000_NCC_CONSISTENT_TOP1",
                                                     CB2000_NCC_CONSISTENT_TOP1_DEFAULT,
                                                     0.0, 1.0);
    gdouble consistent_mean3 = cb2000_ncc_env_double("CB2000_NCC_CONSISTENT_MEAN3",
                                                      CB2000_NCC_CONSISTENT_MEAN3_DEFAULT,
                                                      0.0, 1.0);
    gdouble consistent_grad1 = cb2000_ncc_env_double("CB2000_NCC_CONSISTENT_GRAD1",
                                                      CB2000_NCC_CONSISTENT_GRAD1_DEFAULT,
                                                      0.0, 1.0);
    guint consistent_support_min = cb2000_ncc_env_uint("CB2000_NCC_CONSISTENT_SUPPORT_MIN",
                                                        CB2000_NCC_CONSISTENT_SUPPORT_MIN_DEFAULT,
                                                        1, CB2000_NR_ENROLL_STAGES);
    gdouble single_top1 = cb2000_ncc_env_double("CB2000_NCC_SINGLE_TOP1",
                                                 CB2000_NCC_SINGLE_TOP1_DEFAULT,
                                                 0.0, 1.0);
    gdouble single_top2 = cb2000_ncc_env_double("CB2000_NCC_SINGLE_TOP2",
                                                 CB2000_NCC_SINGLE_TOP2_DEFAULT,
                                                 0.0, 1.0);
    gdouble single_mean3 = cb2000_ncc_env_double("CB2000_NCC_SINGLE_MEAN3",
                                                  CB2000_NCC_SINGLE_MEAN3_DEFAULT,
                                                  0.0, 1.0);
    gdouble single_grad1 = cb2000_ncc_env_double("CB2000_NCC_SINGLE_GRAD1",
                                                  CB2000_NCC_SINGLE_GRAD1_DEFAULT,
                                                  0.0, 1.0);
    guint single_support_min = cb2000_ncc_env_uint("CB2000_NCC_SINGLE_SUPPORT_MIN",
                                                    CB2000_NCC_SINGLE_SUPPORT_MIN_DEFAULT,
                                                    1, CB2000_NR_ENROLL_STAGES);
    gdouble nomatch_top1 = cb2000_ncc_env_double("CB2000_NCC_NOMATCH_TOP1",
                                                 CB2000_NCC_NOMATCH_TOP1_DEFAULT,
                                                 0.0, 1.0);
    gdouble clear_nomatch_top1 = cb2000_ncc_env_double("CB2000_NCC_CLEAR_NOMATCH_TOP1",
                                                        CB2000_NCC_CLEAR_NOMATCH_TOP1_DEFAULT,
                                                        0.0, 1.0);
    gdouble clear_nomatch_mean3 = cb2000_ncc_env_double("CB2000_NCC_CLEAR_NOMATCH_MEAN3",
                                                         CB2000_NCC_CLEAR_NOMATCH_MEAN3_DEFAULT,
                                                         0.0, 1.0);
    guint adaptive_relax_support = cb2000_ncc_env_uint("CB2000_NCC_ADAPTIVE_RELAX_SUPPORT",
                                                        CB2000_NCC_ADAPTIVE_RELAX_SUPPORT_DEFAULT,
                                                        0, 1);
    gdouble relax = cb2000_ncc_compute_adaptive_relax(tel);

    if (relax > 0.0) {
        match_top1 = MAX(0.0, match_top1 - relax);
        match_top2 = MAX(0.0, match_top2 - (relax * 0.75));
        match_mean3 = MAX(0.0, match_mean3 - (relax * 0.75));
        match_grad1 = MAX(0.0, match_grad1 - (relax * 0.50));

        medium_top1 = MAX(0.0, medium_top1 - relax);
        medium_top2 = MAX(0.0, medium_top2 - (relax * 0.80));
        medium_mean3 = MAX(0.0, medium_mean3 - (relax * 0.80));
        medium_grad1 = MAX(0.0, medium_grad1 - (relax * 0.50));

        dominant_top1 = MAX(0.0, dominant_top1 - (relax * 0.40));
        dominant_top2 = MAX(0.0, dominant_top2 - (relax * 0.25));
        dominant_mean3 = MAX(0.0, dominant_mean3 - (relax * 0.50));
        dominant_grad1 = MAX(0.0, dominant_grad1 - (relax * 0.40));

        consistent_top1 = MAX(0.0, consistent_top1 - (relax * 0.60));
        consistent_mean3 = MAX(0.0, consistent_mean3 - (relax * 0.50));
        consistent_grad1 = MAX(0.0, consistent_grad1 - (relax * 0.40));
        single_top1 = MAX(0.0, single_top1 - (relax * 0.20));
        single_top2 = MAX(0.0, single_top2 - (relax * 0.10));
        single_mean3 = MAX(0.0, single_mean3 - (relax * 0.20));
        single_grad1 = MAX(0.0, single_grad1 - (relax * 0.20));

        if (adaptive_relax_support) {
            if (support_min > 1 && relax >= CB2000_NCC_ADAPTIVE_SUPPORT_RELAX_AT)
                support_min--;
            if (medium_support_min > 1 && relax >= CB2000_NCC_ADAPTIVE_SUPPORT_RELAX_AT)
                medium_support_min--;
            if (consistent_support_min > 1 && relax >= CB2000_NCC_ADAPTIVE_SUPPORT_RELAX_AT)
                consistent_support_min--;
            if (single_support_min > 1 && relax >= CB2000_NCC_ADAPTIVE_SUPPORT_RELAX_AT)
                single_support_min--;
        }
    }

    fp_dbg("[ NCC ] classify thresholds: strong(top1>=%.3f top2>=%.3f mean3>=%.3f grad1>=%.3f support>=%u) "
           "medium(top1>=%.3f top2>=%.3f mean3>=%.3f grad1>=%.3f support>=%u) "
           "dominant(top1>=%.3f top2>=%.3f mean3>=%.3f grad1>=%.3f support>=%u) "
           "consistent(top1>=%.3f mean3>=%.3f grad1>=%.3f support>=%u) "
           "single(top1>=%.3f top2>=%.3f mean3>=%.3f grad1>=%.3f support>=%u) "
           "clear_nomatch(support==0 && top1<%.3f && mean3<%.3f) "
           "nomatch(top1<%.3f) "
           "adaptive(relax=%.3f relax_support=%u cohesion_mean=%.3f cohesion_min=%.3f pairs=%u)",
           match_top1, match_top2, match_mean3, match_grad1, support_min,
           medium_top1, medium_top2, medium_mean3, medium_grad1, medium_support_min,
           dominant_top1, dominant_top2, dominant_mean3, dominant_grad1, dominant_support_min,
           consistent_top1, consistent_mean3, consistent_grad1, consistent_support_min,
           single_top1, single_top2, single_mean3, single_grad1, single_support_min,
           clear_nomatch_top1, clear_nomatch_mean3,
           nomatch_top1,
           relax,
           adaptive_relax_support,
           tel->enroll_pair_mean_intensity,
           tel->enroll_pair_min_intensity,
           tel->enroll_pair_count);

    if (tel->compared_images == 0)
        return CB2000_NCC_DECISION_ERROR;

    if (tel->support_count >= support_min &&
        tel->best_intensity >= match_top1 &&
        tel->second_intensity >= match_top2 &&
        tel->mean_top3_intensity >= match_mean3 &&
        tel->grad_for_best_intensity >= match_grad1)
        return CB2000_NCC_DECISION_MATCH;

    if (tel->support_count >= medium_support_min &&
        tel->best_intensity >= medium_top1 &&
        tel->second_intensity >= medium_top2 &&
        tel->mean_top3_intensity >= medium_mean3 &&
        tel->grad_for_best_intensity >= medium_grad1)
        return CB2000_NCC_DECISION_MATCH;

    if (tel->support_count >= dominant_support_min &&
        tel->best_intensity >= dominant_top1 &&
        tel->second_intensity >= dominant_top2 &&
        tel->mean_top3_intensity >= dominant_mean3 &&
        tel->grad_for_best_intensity >= dominant_grad1)
        return CB2000_NCC_DECISION_MATCH;

    if (tel->support_count >= consistent_support_min &&
        tel->best_intensity >= consistent_top1 &&
        tel->mean_top3_intensity >= consistent_mean3 &&
        tel->grad_for_best_intensity >= consistent_grad1)
        return CB2000_NCC_DECISION_MATCH;

    if (tel->support_count >= single_support_min &&
        tel->best_intensity >= single_top1 &&
        tel->second_intensity >= single_top2 &&
        tel->mean_top3_intensity >= single_mean3 &&
        tel->grad_for_best_intensity >= single_grad1)
        return CB2000_NCC_DECISION_MATCH;

    if (tel->support_count == 0 &&
        tel->best_intensity < clear_nomatch_top1 &&
        tel->mean_top3_intensity < clear_nomatch_mean3)
        return CB2000_NCC_DECISION_NO_MATCH;

    if (tel->best_intensity < nomatch_top1)
        return CB2000_NCC_DECISION_NO_MATCH;

    return CB2000_NCC_DECISION_RETRY;
}
 ...
 */
static void
pack_enrollment_data(FpiDeviceCanvasbio *self, FpPrint *print)
{
    ...
    fp_info("Packed %d enrollment images (%zu bytes total)",
            CB2000_NR_ENROLL_STAGES, total_size);
}

/**
 * unpack_and_match - Unpack enrolled images and run robust NCC telemetry
 * @print: Enrolled FpPrint containing stored images
 * @captured: Captured image to match against (CB2000_IMG_SIZE bytes)
 * @tel: Output telemetry for decision matrix
 *
 * Returns TRUE if unpack succeeded, FALSE on format error.
 */
static gboolean
unpack_and_match(FpPrint *print, const guint8 *captured, Cb2000NccTelemetry *tel)
{
    g_autoptr(GVariant) print_data = NULL;
    g_autoptr(GVariant) images_var = NULL;
    gdouble captured_grad[CB2000_IMG_SIZE];
    guint8 captured_mask[CB2000_IMG_SIZE];
    gdouble top_scores[CB2000_NR_ENROLL_STAGES] = {0};
    guint32 version;
    guint16 width, height;
    const guint8 *images_data;
    gsize images_len;
    guint mask_threshold;
    guint min_samples;
    guint support_min;
    gint max_shift;
    gdouble support_int;
    gdouble support_grad;
    gdouble support_int_effective;
    gdouble support_grad_effective;
    gint n_images;
    gint i;
    guint top_n;
    ...
    if (version != 1 || width != CB2000_IMG_WIDTH || height != CB2000_IMG_HEIGHT) {
        fp_warn("Print data version/size mismatch: v=%u w=%u h=%u",
    ...
    tel->best_intensity = -1.0;
    tel->second_intensity = -1.0;
    tel->mean_top3_intensity = -1.0;
    tel->grad_for_best_intensity = 0.0;

    mask_threshold = cb2000_ncc_env_uint("CB2000_NCC_MASK_THRESH",
                                         CB2000_NCC_MASK_THRESHOLD_DEFAULT,
                                         1, 254);
    min_samples = cb2000_ncc_env_uint("CB2000_NCC_MIN_SAMPLES",
                                      CB2000_NCC_MIN_SAMPLES_DEFAULT,
                                      64, CB2000_IMG_SIZE);
    support_min = cb2000_ncc_env_uint("CB2000_NCC_SUPPORT_MIN",
                                      CB2000_NCC_SUPPORT_MIN_DEFAULT,
                                      1, CB2000_NR_ENROLL_STAGES);
    support_int = cb2000_ncc_env_double("CB2000_NCC_SUPPORT_INT",
                                        CB2000_NCC_SUPPORT_INT_DEFAULT,
                                        0.0, 1.0);
    support_grad = cb2000_ncc_env_double("CB2000_NCC_SUPPORT_GRAD",
                                         CB2000_NCC_SUPPORT_GRAD_DEFAULT,
                                         0.0, 1.0);
    max_shift = (gint) cb2000_ncc_env_uint("CB2000_NCC_MAX_SHIFT",
                                           CB2000_NCC_MAX_SHIFT_DEFAULT,
                                           0, 8);

    cb2000_ncc_compute_enroll_cohesion(images_data, n_images,
                                       (guint8) mask_threshold,
                                       min_samples,
                                       max_shift,
                                       tel);
    tel->adaptive_relax = cb2000_ncc_compute_adaptive_relax(tel);
    support_int_effective = MAX(0.0, support_int - (tel->adaptive_relax * 0.50));
    support_grad_effective = MAX(0.0, support_grad - (tel->adaptive_relax * 0.35));

    fp_info("[ NCC ] enroll cohesion: pairs=%u mean_int=%.4f min_int=%.4f mean_grad=%.4f relax=%.4f",
            tel->enroll_pair_count,
            tel->enroll_pair_mean_intensity,
            tel->enroll_pair_min_intensity,
            tel->enroll_pair_mean_gradient,
            tel->adaptive_relax);

    cb2000_build_foreground_mask(captured, captured_mask,
                                 CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT,
                                 (guint8) mask_threshold);
    cb2000_compute_gradient_magnitude(captured, captured_grad,
                                      CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT);

    for (i = 0; i < n_images; i++) {
        const guint8 *enrolled = images_data + i * CB2000_IMG_SIZE;
        gdouble enrolled_grad[CB2000_IMG_SIZE];
        guint8 enrolled_mask[CB2000_IMG_SIZE];
        gdouble int_score;
        gdouble grad_score;

        cb2000_build_foreground_mask(enrolled, enrolled_mask,
                                     CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT,
                                     (guint8) mask_threshold);
        cb2000_compute_gradient_magnitude(enrolled, enrolled_grad,
                                          CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT);

        int_score = cb2000_ncc_match_u8_masked(captured, enrolled,
                                               captured_mask, enrolled_mask,
                                               CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT,
                                               max_shift, min_samples);
        grad_score = cb2000_ncc_match_f64_masked(captured_grad, enrolled_grad,
                                                 captured_mask, enrolled_mask,
                                                 CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT,
                                                 max_shift, min_samples);

        top_scores[i] = int_score;
        tel->compared_images++;

        if (int_score > tel->best_intensity) {
            tel->second_intensity = tel->best_intensity;
            tel->best_intensity = int_score;
            tel->grad_for_best_intensity = grad_score;
        } else if (int_score > tel->second_intensity) {
            tel->second_intensity = int_score;
        }

        if (int_score >= support_int_effective && grad_score >= support_grad_effective)
            tel->support_count++;

        fp_dbg("[ NCC ] gallery=%d int=%.4f grad=%.4f", i, int_score, grad_score);
    }
    ...
    if (top_n > 0) {
        ...
        for (i = 0; i < (gint) top_n; i++)
            sum += top_scores[i];
        tel->mean_top3_intensity = sum / top_n;
    } else {
        tel->mean_top3_intensity = 0.0;
    }

    if (tel->second_intensity < 0.0)
        tel->second_intensity = 0.0;
    if (tel->best_intensity < 0.0)
        tel->best_intensity = 0.0;

    fp_info("[ NCC ] top1=%.4f top2=%.4f mean3=%.4f grad1=%.4f support=%u/%u "
            "mask_thresh=%u min_samples=%u max_shift=%d support_min=%u "
            "support_int=%.3f(%.3f) support_grad=%.3f(%.3f) adapt=%.3f",
            tel->best_intensity,
            tel->second_intensity,
            tel->mean_top3_intensity,
            tel->grad_for_best_intensity,
            tel->support_count,
            tel->compared_images,
            mask_threshold,
            min_samples,
            ...
            support_min,
            support_int,
            support_int_effective,
            support_grad,
            support_grad_effective,
            tel->adaptive_relax);
...
cycle_run_state(FpiSsm *ssm, FpDevice *dev)
{
    ...
    case CYCLE_VERIFY_RESULT_STATUS:
    {
        ...
        if (gate_retry) {
            ...
            fp_info("[ VERIFY_RESULT ] attempt=%u decision=%s result_class=%s status=0x%02x result=0x%02x route=%s experimental=%d status_short=%d",
                    attempt,
                    ...
                    self->experimental_device_assisted_verify,
                    self->verify_device_status_short_circuit);
        ...
        switch (self->verify_ack_decision_last) {
        case CB2000_VERIFY_ACK_RETRY:
            /* Retry already handled by finalize_ack_gate; fall through to NBIS. */

        case CB2000_VERIFY_ACK_DEVICE_NOMATCH:
            if (self->verify_device_status_short_circuit ||
                self->experimental_device_assisted_verify) {
            ...
            self->verify_route_last = CB2000_VERIFY_ROUTE_HOST_NBIS;
            fp_info("[ VERIFY_RESULT ] attempt=%u decision=%s result_class=%s status=0x%02x result=0x%02x route=%s experimental=%d status_short=%d",
                    attempt,
                    cb2000_verify_ack_decision_label(self->verify_ack_decision_last),
                    cb2000_verify_result_class_label(self->verify_result_class_last),
        ...
        case CB2000_VERIFY_ACK_READY:
            if (self->experimental_device_assisted_verify) {
                self->verify_route_last = CB2000_VERIFY_ROUTE_DEVICE_STATUS;
            } else {
                self->verify_route_last = CB2000_VERIFY_ROUTE_HOST_NBIS;
            }
            fp_info("[ VERIFY_RESULT ] attempt=%u decision=%s result_class=%s status=0x%02x result=0x%02x route=%s experimental=%d status_short=%d",
                    ...
                    self->experimental_device_assisted_verify,
                    self->verify_device_status_short_circuit);
            if (self->verify_ready_query_enabled) {
    ...
    case CYCLE_VERIFY_READY_QUERY_A:
    {
        ...
        if (!cb2000_is_verify_phase_action(action) || !self->verify_ready_query_enabled) {
            fpi_ssm_jump_to_state(ssm, CYCLE_SUBMIT_IMAGE);
            break;
        }
    ...
    case CYCLE_VERIFY_READY_QUERY_B:
    {
        ...
        if (!self->verify_ready_query_b_enabled) {
            fp_dbg("[ VERIFY_READY_QUERY_B ] disabled -> skipping");
            fpi_ssm_next_state(ssm);
            break;
        }
    ...
    case CYCLE_VERIFY_READY_DECIDE:
    {
        ...
        gboolean has_b = self->verify_ready_query_b_enabled &&
                         (self->verify_ready_query_b_len > 0);
        ...
        case CB2000_VERIFY_READY_MATRIX_READY:
            ...
            /* Keep conservative fallback to NBIS until MoC decision table is closed. */
            if (self->verify_route_last == CB2000_VERIFY_ROUTE_DEVICE_STATUS)
                self->verify_route_last = CB2000_VERIFY_ROUTE_HOST_NBIS;
            fpi_ssm_next_state(ssm);
            break;
    ...
    case CYCLE_SUBMIT_IMAGE:
        ...
        /* Create and submit image to libfprint for MoH processing. */
        {
            ...
            FpiDeviceAction action = fpi_device_get_current_action(dev);
            gboolean use_bg_subtract = cb2000_should_apply_bg_subtraction(action);
            ...
            /* CLAHE: local contrast enhancement on raw image before upscale.
             * This is the key preprocessing for NBIS on small sensors. */
            ...
            double variance_norm = calculate_variance(img->data,
                                                      CB2000_IMG_WIDTH,
                                                      CB2000_IMG_HEIGHT);
            ...
            const int area_min_pct = cb2000_get_area_min_pct(action);
            ...
            if (self->previous_norm_valid && self->previous_norm_buffer) {
                overlap_pct = calculate_overlap_percent(img->data,
            ...
            gboolean fail_area = (area_pct < area_min_pct);
            gboolean fail_variance = (variance_norm < CB2000_MIN_IMAGE_VARIANCE);
            gboolean fail_focus = (focus_var < CB2000_MIN_FOCUS_VARIANCE);

            if (fail_area) {
                fp_warn("Reject! quality.fingerprint_area %d < %d (threshold)",
                        area_pct, area_min_pct);
            }
            ...
            if (fail_variance || fail_area || fail_focus) {
                ...
                fp_info("[ GATE ] action=%s min_area=%d fail_var=%d fail_area=%d fail_focus=%d",
                        cb2000_action_label(action), area_min_pct,
                        fail_variance, fail_area, fail_focus);
                if (self->previous_norm_buffer) {
                    memcpy(self->previous_norm_buffer, img->data, CB2000_IMG_SIZE);
            ...
            /* Optional upscale to improve NBIS minutiae extraction on small images. */
            int upscale = cb2000_get_upscale_factor(action);
            ...
            if (upscale > 1) {
                ...
                if (resized) {
                    ...
                    /* Smooth bilinear upscale artifacts for NBIS ridge detection. */
            ...
            if (cb2000_dump_binarized_enabled()) {
                ...
                dbg->ppmm = img->ppmm;
                dbg->flags = img->flags;
                memcpy(dbg->data, img->data, CB2000_IMG_SIZE);
            ...
            gboolean precheck_enabled = cb2000_should_precheck_minutiae(action);
            ...
            /* Action-specific handling (V33 FpDevice + NCC) */
            {
                if (action == FPI_DEVICE_ACTION_CAPTURE) {
                    ...
                    memcpy(norm, img->data, CB2000_IMG_SIZE);
                    normalize_contrast(norm, CB2000_IMG_WIDTH, CB2000_IMG_HEIGHT);
                    ...
                    Cb2000NccTelemetry ncc_tel;
                    Cb2000NccDecision ncc_decision = CB2000_NCC_DECISION_ERROR;
                    ...
                    if (enrolled && unpack_and_match(enrolled, norm, &ncc_tel)) {
                        ncc_decision = cb2000_ncc_classify(&ncc_tel);
                        switch (ncc_decision) {
                        case CB2000_NCC_DECISION_MATCH:
                    ...
                    fp_info("[ NCC ] decision=%s -> result=%s",
                            cb2000_ncc_decision_label(ncc_decision),
...
cycle_complete(FpiSsm *ssm, FpDevice *dev, GError *error)
{
    ...
    fp_dbg("[ STATS ] accepted=%u retry_total=%u bg=%u area=%u quality=%u precheck=%u dev_status=%u "
           ...
           "result_class=%s rc_ready=%u rc_retry=%u rc_nomatch=%u rc_unknown=%u status_short=%d "
           "ready_query_en=%d ready_query_total=%u ready_query_status=0x%02x "
           "ready_query_b_en=%d ready_query_b_total=%u ready_query_b_status=0x%02x "
           ...
           self->verify_result_class_count[CB2000_VERIFY_RESULT_CLASS_UNKNOWN],
           self->verify_device_status_short_circuit,
           self->verify_ready_query_enabled,
           self->verify_ready_query_total,
           self->verify_ready_query_status_last,
           self->verify_ready_query_b_enabled,
           self->verify_ready_query_b_total,
...
{
    FpiDeviceCanvasbio *self = FPI_DEVICE_CANVASBIO(device);
    GUsbDevice *usb_dev = fpi_device_get_usb_device(device);
    GError *error = NULL;

    fp_info("Opening CanvasBio CB2000 device");
    ...
    self->experimental_device_assisted_verify = FALSE;
    ...
    self->verify_device_status_short_circuit = TRUE;
    self->verify_ready_query_enabled = TRUE;
    self->verify_ready_query_b_enabled = TRUE;
 ...
 */
static void
dev_action_common_init(FpiDeviceCanvasbio *self)
{
    self->deactivating = FALSE;
    self->deactivation_in_progress = FALSE;
    ...
    self->verify_device_status_short_circuit = TRUE;
    self->experimental_device_assisted_verify =
        cb2000_get_env_bool("CB2000_EXPERIMENTAL_DEVICE_ASSISTED_VERIFY", FALSE);
    self->verify_device_status_short_circuit =
        cb2000_get_env_bool("CB2000_VERIFY_DEVICE_STATUS_SHORT_CIRCUIT", TRUE);
    self->verify_ready_query_enabled =
        cb2000_get_env_bool("CB2000_VERIFY_READY_QUERY", TRUE);
    self->verify_ready_query_b_enabled =
        cb2000_get_env_bool("CB2000_VERIFY_READY_QUERY_B", TRUE);
    self->verify_ready_query_total = 0;
    self->verify_ready_query_status_last = 0x00;
    self->verify_ready_query_len = 0;
    memset(self->verify_ready_query_data, 0, sizeof(self->verify_ready_query_data));
...
dev_verify(FpDevice *device)
{
    ...
    fp_info("Starting verification (NCC)");
    dev_action_common_init(self);
    self->verify_result = FPI_MATCH_FAIL;
...
static void
fpi_device_canvasbio_init(FpiDeviceCanvasbio *self)
{
}

static void
fpi_device_canvasbio_class_init(FpiDeviceCanvasbioClass *klass)
{
